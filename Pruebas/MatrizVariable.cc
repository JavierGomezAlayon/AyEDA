...
MatrizVariable::MatrizVariable(std::pair<int,int> tamano) {
  ...
  for (int i = 0; i < tamano.first; i++) {
    matriz_[i] = VectorVariable(tamano.second);
  }
...
void MatrizVariable::AumentarTamano(const int filas, const int columnas) {
  if (filas == 1) {
    matriz_.push_back(VectorVariable(matriz_.at(0).getTamano()));
  } else if (filas == -1) {
    matriz_.insert(matriz_.begin(), VectorVariable(matriz_.at(0).getTamano()));
    referencia0_filas_++;
  }
  if (columnas == 1) {
    for (int i = 0; i < matriz_.size(); i++) {
      matriz_.at(i).AumentarTamano(true);
    }
  } else if (columnas == -1) {
    for (int i = 0; i < matriz_.size(); i++) {
      matriz_.at(i).AumentarTamano(false);
    }
  }
  return;
}
